...
# Top side
INPUT = ttk.Frame(ROOT, width=FULLWIDTH,
                  height=FULLHEIGHT / 2, padding='10 10 5 10')
# Bottom side
OUTPUT = ttk.Frame(ROOT, width=FULLWIDTH,
                   height=FULLHEIGHT / 2, padding='10 0 10 10')
...
# Output image host
OUTPUT_IMAGE = funcs.img_holder(
    OUTPUT, bg='#333', width=funcs.wwidth(OUTPUT) / 2)

# Get the resultant image btn
OUTPUT_GETBTN = tk.Button(
    OUTPUT, text='Get the result image',
    command=lambda: green_screen_alogrithm(
        INPUT_BG_IMAGE, INPUT_FR_IMAGE, OUTPUT_IMAGE),
    font=FONT
)
# Downlaod image btn
...
# Download button
OUTPUT_DOWNLOBTN = tk.Button(
    OUTPUT_BOTTOM, text='Download the result',
    command=lambda: funcs.download_frame_img(
        OUTPUT_IMAGE, OUTPUT_DOWNLOADAS.get('1.0', 'end')),
    font=FONT
)
# Download as filename getter
OUTPUT_DOWNLOADAS = tk.Text(
    OUTPUT_BOTTOM, width=25, height=1,
    font=FONT
)
# Download as label text
...
OUTPUT_DOWNLOBTN.grid(column=0, row=0, padx=10)
